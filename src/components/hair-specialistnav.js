  ...
  return (
    <nav className="sticky top-0 z-50 backdrop-blur-xl bg-white/70 dark:bg-black/70 border-b border-rose-200 dark:border-stone-800">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
        ...
        <Link href={getHairSpecialistStoreUrl(hairSpecialist?.slug)} className="flex items-center gap-3">
          ...
          <span className="text-2xl font-extrabold text-stone-700 tracking-tight">{hairSpecialist?.name}</span>
        </Link>

        <div className="hidden md:flex items-center gap-3">
          <Star className="h-4 w-4 text-rose-400 dark:text-rose-300 animate-pulse" />
          <div key={index} className="mx-2 text-sm font-medium bg-gradient-to-r from-rose-600 via-rose-500 to-amber-400 text-transparent bg-clip-text transition-opacity duration-500" aria-live="polite">
            {messages[index]}
          </div>
          <Star className="h-4 w-4 text-rose-400 dark:text-rose-300 animate-pulse" />
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="hidden sm:block text-sm text-stone-700 dark:text-rose-200 font-medium">{hairSpecialist?.name}</div>
          <ModeToggle />
          <Link
            href={getHairSpecialistBookingUrl(hairSpecialist?.slug)}
