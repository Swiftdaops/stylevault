...
import { API_BASE_URL, getNailServiceCatalog } from '@/lib/nail-technician-api'
...
export default function NailServiceForm({ nailTechnician, mode = 'create', initialValue = null, onCancel, onSubmit, saving = false, error = '' }) {
  ...
  useEffect(() => {
    async function loadCatalog() {
      ...
      const data = await getNailServiceCatalog()
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 rounded-3xl border border-fuchsia-200/70 bg-white/90 p-6 shadow-sm dark:border-stone-800 dark:bg-stone-950/70">
      <div className="flex items-center justify-between gap-4">
        <div>
          <div className="text-lg font-semibold">{mode === 'edit' ? 'Edit service' : 'Create service'}</div>
          <div className="text-sm text-stone-600 dark:text-fuchsia-300">Add nail services, pricing tiers, and add-ons for your booking page.</div>
        </div>
      ...
      <div>
        ...
        <Select value={form.catalogId} onValueChange={handleCatalogChange}>
          ...
          <SelectContent className="bg-fuchsia-50">
            {catalog.map((item) => (
              <SelectItem key={item._id} value={item._id} className="bg-fuchsia-50">{item.category} — {item.name}</SelectItem>
            ))}
          </SelectContent>
        </Select>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_220px]">
        <div className="space-y-4">
              ...
              <Select value={form.category} onValueChange={(v) => setForm((current) => ({ ...current, category: v }))}>
                ...
                <SelectContent className="bg-fuchsia-50">
                  {categoryOptions.map((option) => (
                    <SelectItem key={option} value={option} className="bg-fuchsia-50">{option}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium">Starting price ({nailTechnician?.currency || 'USD'})</label>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <textarea className={`${textareaClassName} min-h-28 text-sm`} value={form.pricingOptionsText} onChange={(e) => setForm((current) => ({ ...current, pricingOptionsText: e.target.value }))} placeholder="Short Set|45.00|75\nLong Set|65.00|105" />
              <p className="mt-1 text-xs text-stone-500 dark:text-fuchsia-400">Format: Label|Price|Duration minutes</p>
            </div>
            <div>
              <label className="block text-sm font-medium">Add-ons</label>
              <textarea className={`${textareaClassName} min-h-28 text-sm`} value={form.addOnsText} onChange={(e) => setForm((current) => ({ ...current, addOnsText: e.target.value }))} placeholder="Chrome Finish|10.00|10\nStone Art|15.00|15" />
              <p className="mt-1 text-xs text-stone-500 dark:text-fuchsia-400">Format: Name|Price|Extra minutes</p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mt-1 flex items-center gap-3">
              ...
              <Button type="button" className="border-2 border-fuchsia-300 bg-white text-stone-950 hover:bg-fuchsia-50 dark:border-stone-700 dark:bg-stone-900 dark:text-fuchsia-300" onClick={() => fileInputRef.current && fileInputRef.current.click()}>
        ...
        <div className="space-y-3 rounded-2xl border border-fuchsia-200/70 bg-fuchsia-50/80 p-4 dark:border-stone-800 dark:bg-stone-950">
          <div className="text-sm font-medium">Preview</div>
          <div className="overflow-hidden rounded-lg border border-fuchsia-200/70 bg-white dark:border-stone-800 dark:bg-stone-900">
            {previewImage ? (
          ...
          {selectedCatalogItem?.image && <div className="text-xs text-stone-500 dark:text-fuchsia-400">Template image available; your sample image will override it publicly.</div>}
        </div>
      </div>
