...
import { API_BASE_URL, getLashServiceCatalog } from '@/lib/lash-technician-api'
...
export default function LashServiceForm({ lashTechnician, mode = 'create', initialValue = null, onCancel, onSubmit, saving = false, error = '' }) {
  ...
  useEffect(() => {
    async function loadCatalog() {
      ...
      const data = await getLashServiceCatalog()
  ...
  const selectedCatalogItem = useMemo(() => catalog.find((item) => item._id === form.catalogId) || null, [catalog, form.catalogId])
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 rounded-3xl border border-violet-200/70 bg-white/90 p-6 shadow-sm dark:border-stone-800 dark:bg-stone-950/70">
      <div className="flex items-center justify-between gap-4">
        <div>
          <div className="text-lg font-semibold">{mode === 'edit' ? 'Edit service' : 'Create service'}</div>
          <div className="text-sm text-stone-600 dark:text-violet-300">Add lash services, pricing tiers, and add-ons for your booking page.</div>
        </div>
      ...
      <div>
        ...
        <Select value={form.catalogId} onValueChange={handleCatalogChange}>
          ...
          <SelectContent className="bg-violet-50">
            {catalog.map((item) => (
              <SelectItem key={item._id} value={item._id} className="bg-violet-50">{item.category} — {item.name}</SelectItem>
            ))}
          </SelectContent>
        </Select>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_220px]">
        <div className="space-y-4">
              ...
              <Select value={form.category} onValueChange={(v) => setForm((current) => ({ ...current, category: v }))}>
                ...
                <SelectContent className="bg-violet-50">
                  {categoryOptions.map((option) => (
                    <SelectItem key={option} value={option} className="bg-violet-50">{option}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium">Starting price ({lashTechnician?.currency || 'USD'})</label>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <textarea className={`${textareaClassName} min-h-28 text-sm`} value={form.pricingOptionsText} onChange={(e) => setForm((current) => ({ ...current, pricingOptionsText: e.target.value }))} placeholder="Classic Set|45.00|75\nHybrid Set|65.00|105" />
              <p className="mt-1 text-xs text-stone-500 dark:text-violet-400">Format: Label|Price|Duration minutes</p>
            </div>
            <div>
              <label className="block text-sm font-medium">Add-ons</label>
              <textarea className={`${textareaClassName} min-h-28 text-sm`} value={form.addOnsText} onChange={(e) => setForm((current) => ({ ...current, addOnsText: e.target.value }))} placeholder="Lash Bath|10.00|10\nColor Accent|15.00|15" />
              <p className="mt-1 text-xs text-stone-500 dark:text-violet-400">Format: Name|Price|Extra minutes</p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mt-1 flex items-center gap-3">
              ...
              <Button type="button" className="border-2 border-violet-300 bg-white text-stone-950 hover:bg-violet-50 dark:border-stone-700 dark:bg-stone-900 dark:text-violet-300" onClick={() => fileInputRef.current && fileInputRef.current.click()}>
            ...
            <p className="mt-1 text-xs text-stone-500 dark:text-violet-400">This image is shown on your public lash booking page. JPG, PNG, WEBP, or SVG up to {MAX_IMAGE_SIZE_LABEL}.</p>
        ...
        <div className="space-y-3 rounded-2xl border border-violet-200/70 bg-violet-50/80 p-4 dark:border-stone-800 dark:bg-stone-950">
          <div className="text-sm font-medium">Preview</div>
          <div className="overflow-hidden rounded-lg border border-violet-200/70 bg-white dark:border-stone-800 dark:bg-stone-900">
            {previewImage ? (
          ...
          {selectedCatalogItem?.image && <div className="text-xs text-stone-500 dark:text-violet-400">Template image available; your sample image will override it publicly.</div>}
        </div>
      </div>
