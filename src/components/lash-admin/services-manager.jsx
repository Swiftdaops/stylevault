...
export default function LashServicesManager() {
  ...
  const loadServices = async () => {
    setLoading(true)
    const data = await getMyLashServices()
  ...
  const handleSubmit = async (payload) => {
    ...
    try {
      if (editingId) {
        await updateMyLashService(editingId, payload)
      } else {
        await createMyLashService(payload)
      }
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-stone-600 dark:text-violet-300">Manage the lash services visible on your booking page.</p>
        </div>
      ...
      <div className="space-y-4">
        {loading && <div className="rounded-xl border border-violet-200/70 bg-white/90 p-5 dark:border-stone-800 dark:bg-black/60">Loading services…</div>}
        {!loading && services.length === 0 && <div className="rounded-xl border border-dashed border-violet-200/70 bg-white/90 p-5 text-sm dark:border-stone-800 dark:bg-black/60">No services created yet.</div>}

        {services.map((service) => (
          <article key={service._id} className="rounded-xl border border-violet-200/70 bg-white/90 p-5 dark:border-stone-800 dark:bg-black/60">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div className="space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  <h2 className="text-lg font-semibold">{service.name}</h2>
                  {service.category ? <span className="rounded-full bg-violet-100 px-2 py-1 text-xs text-stone-800">{service.category}</span> : null}
                </div>
                <p className="text-sm text-stone-600 dark:text-violet-300">{service.description || 'No description added yet.'}</p>
                <div className="flex flex-wrap gap-2 text-xs">
                  <span className="rounded-full bg-stone-100 px-2 py-1 text-stone-800">{service.duration} min</span>
                  <span className="rounded-full bg-stone-100 px-2 py-1 text-stone-800">{formatCurrency(service.price, lashTechnician?.currency || 'USD')}</span>
