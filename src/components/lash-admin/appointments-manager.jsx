...
import { formatCurrency, getMyLashAppointments, updateMyLashAppointment } from '@/lib/lash-technician-api'
...
export default function LashAppointmentsManager() {
  ...
  const loadAppointments = useCallback(async () => {
    setLoading(true)
    const data = await getMyLashAppointments()
  ...
  }, [lashTechnician?._id, loadAppointments])
  ...
  const saveEdit = async () => {
    await updateMyLashAppointment(editingId, draft)
    setEditingId(null)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-stone-600 dark:text-violet-300">View and update only your own lash bookings.</p>
        </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[360px_1fr]">
        <div className="rounded-xl border border-violet-200/70 bg-white/90 p-5 dark:border-stone-800 dark:bg-black/60">
          <div className="mb-3 flex items-center justify-between gap-4">
            <div>
              <div className="text-lg font-semibold">Booking calendar</div>
              <div className="text-sm text-stone-600 dark:text-violet-300">Booked dates are highlighted.</div>
            </div>
          ...
          <div className="mt-4 rounded-lg border border-violet-200/70 bg-violet-50/80 p-3 text-sm text-stone-700 dark:border-stone-800 dark:bg-stone-950 dark:text-violet-200">
            ...
            <div className="mt-2 text-xs text-stone-500 dark:text-violet-400">{filteredAppointments.length} appointment(s) match your current filters.</div>
          </div>
        </div>

        <div className="space-y-4">
          {loading && <div className="rounded-xl border border-violet-200/70 bg-white/90 p-5 dark:border-stone-800 dark:bg-black/60">Loading appointments…</div>}
          {!loading && filteredAppointments.length === 0 && <div className="rounded-xl border border-dashed border-violet-200/70 bg-white/90 p-5 text-sm dark:border-stone-800 dark:bg-black/60">No appointments found for the selected filters.</div>}

          {filteredAppointments.map((appointment) => (
            <div key={appointment._id} className="rounded-xl border border-violet-200/70 bg-white/90 p-5 dark:border-stone-800 dark:bg-black/60">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  <h2 className="text-lg font-semibold">{appointment.service?.name || 'Service'} — {appointment.customerName}</h2>
                  <div className="mt-1 text-sm text-stone-600 dark:text-violet-300">{appointment.customerEmail}</div>
                  <div className="mt-2 flex flex-wrap gap-2 text-xs">
                    <span className="rounded-full bg-violet-100 px-2 py-1 text-stone-800">{appointment.date}</span>
                    <span className="rounded-full bg-violet-100 px-2 py-1 text-stone-800">{appointment.time}</span>
                    <span className="rounded-full bg-violet-100 px-2 py-1 text-stone-800">{appointment.status}</span>
                ...
                <div className="text-sm font-medium">{formatCurrency(appointment.price, lashTechnician?.currency || 'USD')}</div>
              </div>
