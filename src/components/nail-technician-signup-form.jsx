...
import { registerNailTechnician, slugify } from '@/lib/nail-technician-api'
...
export default function NailTechnicianSignupForm({ initialCountry = 'CA' }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const slug = slugify(name || email.split('@')[0] || 'nail-technician')
  ...
  return (
    <div className="mx-auto max-w-md rounded-2xl border border-fuchsia-200/70 bg-white/90 p-6 shadow-sm dark:border-stone-800 dark:bg-black/70">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-stone-700 dark:text-fuchsia-200">Full name</label>
          <input type="text" required value={name} onChange={(e) => {
            const nextValue = e.target.value
            setName(nextValue)
            if (fieldErrors.name) validateField('name', { ...values, name: nextValue })
          }} onBlur={() => validateField('name')} className="mt-1 w-full rounded-md border px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-fuchsia-300 dark:border-stone-700 dark:bg-stone-900" aria-invalid={Boolean(fieldErrors.name)} />
        ...
        <div>
          ...
          }} onBlur={() => validateField('email')} className="mt-1 w-full rounded-md border px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-fuchsia-300 dark:border-stone-700 dark:bg-stone-900" aria-invalid={Boolean(fieldErrors.email)} />
