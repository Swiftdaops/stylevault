...
import { checkNailTechnicianEmailAvailability, registerNailTechnician } from '@/lib/nail-technician-api'
...
export default function NailTechnicianSignupForm({ initialCountry = 'CA' }) {
  ...
  const { emailAvailability, checkEmailAvailability, resetEmailAvailability } = useEmailAvailability({
    checkAvailability: checkNailTechnicianEmailAvailability,
    setFieldError,
  })
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const slug = slugifyBrandName(name || email.split('@')[0] || 'nail-technician')
      await registerNailTechnician({
        name,
        email,
        password,
        slug,
  ...
  return (
    <div className="mx-auto max-w-md rounded-2xl border border-fuchsia-200/70 bg-white/90 p-6 shadow-sm dark:border-stone-800 dark:bg-black/70">
      <h2 className="mb-4 text-2xl font-semibold">Nail Technician Sign up</h2>
      <form onSubmit={handleSubmit} className="space-y-4">
        <BrandNameInput
          ...
          label="Brand name"
          labelClassName="block text-sm font-medium text-stone-700 dark:text-fuchsia-200"
          inputClassName="rounded-md border px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-fuchsia-300 dark:border-stone-700 dark:bg-stone-900"
          previewClassName="mt-1 text-xs text-stone-500 dark:text-fuchsia-300"
          suggestionsClassName="mt-2 flex flex-wrap gap-2"
          suggestionButtonClassName="rounded-full border border-fuchsia-200 px-3 py-1 text-xs font-medium text-stone-700 transition hover:bg-fuchsia-50 dark:border-stone-700 dark:text-fuchsia-200 dark:hover:bg-stone-900"
          slugPreview={slugPreview || 'your-brand-name'}
          placeholder="e.g. Pretty Tips Studio"
        />
        ...
        <div>
          ...
          }} className="mt-1 w-full rounded-md border px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-fuchsia-300 dark:border-stone-700 dark:bg-stone-900" aria-invalid={Boolean(fieldErrors.email)} />
          {fieldErrors.email ? <p className="mt-1 text-xs text-destructive">{fieldErrors.email}</p> : null}
          {!fieldErrors.email && emailAvailability.message ? <p className={`mt-1 text-xs ${emailAvailability.available ? 'text-emerald-600' : 'text-stone-500 dark:text-fuchsia-300'}`}>{emailAvailability.message}</p> : null}
        </div>
        ...
        <PasswordInput
          ...
          labelClassName="block text-sm font-medium text-stone-700 dark:text-fuchsia-200"
          inputClassName="w-full rounded-md border px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-fuchsia-300 dark:border-stone-700 dark:bg-stone-900"
        ...
        <div>
          <label className="block text-sm font-medium text-stone-700 dark:text-fuchsia-200">Country</label>
          <Select value={country} onValueChange={(nextCountry) => {
            ...
            <SelectTrigger className="mt-1 h-10 w-full rounded-md border border-fuchsia-200 bg-white px-3 dark:border-stone-700 dark:bg-stone-900">
              <SelectValue placeholder="Select country" />
            </SelectTrigger>
            <SelectContent className="bg-fuchsia-50">
              {countryOptions.map((option) => (
                <SelectItem key={option.code} value={option.code} className="bg-fuchsia-50">{option.label}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
        ...
        <PhoneNumberInput
          ...
          labelClassName="block text-sm font-medium text-stone-700 dark:text-fuchsia-200"
          wrapperClassName="border-fuchsia-200 dark:border-stone-700 dark:bg-stone-900"
          prefixClassName="bg-fuchsia-50 text-stone-700 dark:border-stone-700 dark:bg-stone-950 dark:text-fuchsia-200"
          inputClassName="bg-white focus:ring-fuchsia-300 dark:bg-stone-900"
          hintClassName="mt-1 text-xs text-stone-500 dark:text-fuchsia-300"
          errorText={fieldErrors.whatsapp}
        />

        <div>
          <label className="block text-sm font-medium text-stone-700 dark:text-fuchsia-200">Currency</label>
          ...
          <p className="mt-1 text-xs text-stone-500 dark:text-fuchsia-300">Currency updates automatically from the selected country.</p>
        </div>

        <div>
          <label className="block text-sm font-medium text-stone-700 dark:text-fuchsia-200">Location</label>
          <input type="text" value={location} onChange={(e) => {
            const nextValue = e.target.value
            setLocation(nextValue)
            if (fieldErrors.location) validateField('location', { ...values, location: nextValue })
          }} onBlur={() => validateField('location')} className="mt-1 w-full rounded-md border px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-fuchsia-300 dark:border-stone-700 dark:bg-stone-900" placeholder="Lekki, Nigeria" aria-invalid={Boolean(fieldErrors.location)} />
          {fieldErrors.location ? <p className="mt-1 text-xs text-destructive">{fieldErrors.location}</p> : null}
        </div>

        <div>
          <label className="block text-sm font-medium text-stone-700 dark:text-fuchsia-200">Specialties</label>
          ...
          }} onBlur={() => validateField('specialties')} className="mt-1 w-full rounded-md border px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-fuchsia-300 dark:border-stone-700 dark:bg-stone-900" placeholder="Gel nails, Acrylic sets, Nail art" aria-invalid={Boolean(fieldErrors.specialties)} />
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="text-sm text-stone-600 dark:text-fuchsia-300">Already have an account? <Link href="/nail-technicians/login" className="text-primary underline">Sign in</Link></div>
