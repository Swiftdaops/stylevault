...
export default function AppointmentsManager() {
  ...
  const saveEdit = async () => {
    await updateMyAppointment(editingId, draft)
    setEditingId(null)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-stone-600 dark:text-amber-300">View and update only your own bookings.</p>
        </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[360px_1fr]">
        <div className="rounded-xl border border-orange-200/60 bg-white/80 p-5 dark:border-stone-800 dark:bg-black/60">
          <div className="mb-3 flex items-center justify-between gap-4">
            <div>
              <div className="text-lg font-semibold">Booking calendar</div>
              <div className="text-sm text-stone-600 dark:text-amber-300">Booked dates are highlighted.</div>
            </div>
          ...
          <div className="mt-4 rounded-lg border border-orange-200/60 bg-orange-50/80 p-3 text-sm text-stone-700 dark:border-stone-800 dark:bg-stone-950 dark:text-amber-200">
            ...
            <div className="mt-2 text-xs text-stone-500 dark:text-amber-400">{filteredAppointments.length} appointment(s) match your current filters.</div>
          </div>
        </div>

        <div className="space-y-4">
          {loading && <div className="rounded-xl border border-orange-200/60 bg-white/80 p-5 dark:border-stone-800 dark:bg-black/60">Loading appointments…</div>}
          {!loading && filteredAppointments.length === 0 && <div className="rounded-xl border border-dashed border-orange-200/60 bg-white/80 p-5 text-sm dark:border-stone-800 dark:bg-black/60">No appointments found for the selected filters.</div>}

          {filteredAppointments.map((appointment) => (
            <div key={appointment._id} className="rounded-xl border border-orange-200/60 bg-white/80 p-5 dark:border-stone-800 dark:bg-black/60">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  <h2 className="text-lg font-semibold">{appointment.service?.name || 'Service'} — {appointment.customerName}</h2>
                  <div className="mt-1 text-sm text-stone-600 dark:text-amber-300">{appointment.customerEmail}</div>
                  <div className="mt-2 flex flex-wrap gap-2 text-xs">
                    <span className="rounded-full bg-orange-100 px-2 py-1 text-stone-800">{appointment.date}</span>
                    <span className="rounded-full bg-orange-100 px-2 py-1 text-stone-800">{appointment.time}</span>
                    <span className="rounded-full bg-orange-100 px-2 py-1 text-stone-800">{appointment.status}</span>
                  </div>
                </div>

                <div className="text-sm font-medium">{formatCurrency(appointment.price, barber?.currency || 'USD')}</div>
              </div>
