...
import { formatCurrency, getMyHairAppointments, getMyHairCustomers, getMyHairServices } from '@/lib/hair-specialist-api'
...
export default function HairSpecialistDashboard() {
  ...
  const profileUrl = useMemo(() => {
    ...
    return getHairSpecialistStoreUrl(hairSpecialist.slug)
  }, [hairSpecialist])
  ...
  useEffect(() => {
    ...
    async function init() {
      if (!hairSpecialist) {
        if (!authLoading) router.push('/hair-specialists/login')
        return
      }
  ...
  }, [hairSpecialist, authLoading, loadDashboardData, router])
  ...
  }, [hairSpecialist?._id, loadDashboardData])
      ...
      {hairSpecialist?.slug && (
        <div className="mt-4 flex items-center justify-between gap-4">
          ...
          <button
            type="button"
            onClick={handleCopy}
            className={`inline-flex items-center gap-2 rounded-md border px-3 py-1.5 text-sm font-medium transition-shadow ${copied ? 'border-emerald-200 bg-emerald-100 text-emerald-800 shadow-sm' : 'border-rose-200/70 bg-rose-50 text-stone-900 hover:shadow-sm'}`}
          >
        ...
        <div className="rounded-lg border border-rose-200/70 bg-white/90 p-4 dark:border-stone-800 dark:bg-black/70 md:col-span-2">
          ...
          <ul className="mt-3 space-y-3">
            {appointments.length === 0 && (
              <li className="text-sm text-stone-600 dark:text-rose-300">No upcoming appointments</li>
            )}
            ...
            {appointments.slice(0, 8).map((appointment) => (
              <li key={appointment._id || appointment.id} className="flex items-center justify-between rounded-md p-3 hover:bg-rose-50 dark:hover:bg-stone-900">
                ...
                <div className="text-sm text-stone-700 dark:text-rose-200">{typeof appointment.price === 'number' ? formatCurrency(appointment.price, hairSpecialist?.currency || 'USD') : ''}</div>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="rounded-lg border border-rose-200/70 bg-white/90 p-4 dark:border-stone-800 dark:bg-black/70 md:col-span-3">
          ...
          <ul className="mt-4 space-y-3">
            {selectedDateAppointments.length === 0 && (
              <li className="rounded-md border border-dashed border-rose-200/70 p-4 text-sm text-stone-600 dark:border-stone-800 dark:text-rose-300">
            ...
            {selectedDateAppointments.map((appointment) => (
              <li key={appointment._id || appointment.id} className="flex items-center justify-between rounded-md border border-rose-200/70 p-3 dark:border-stone-800">
                ...
                <div className="text-sm text-stone-700 dark:text-rose-200">{formatCurrency(appointment.price, hairSpecialist?.currency || 'USD')}</div>
              </li>
            ))}
          </ul>
        </div>
