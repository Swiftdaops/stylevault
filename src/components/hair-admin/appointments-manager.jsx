...
import { formatCurrency, getMyHairAppointments, updateMyHairAppointment } from '@/lib/hair-specialist-api'
...
export default function HairAppointmentsManager() {
  ...
  const loadAppointments = useCallback(async () => {
    setLoading(true)
    const data = await getMyHairAppointments()
  ...
  const saveEdit = async () => {
    await updateMyHairAppointment(editingId, draft)
    setEditingId(null)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-stone-600 dark:text-rose-300">View and update only your own salon bookings.</p>
        </div>
          ...
          {filteredAppointments.map((appointment) => (
            <div key={appointment._id} className="rounded-xl border border-rose-200/70 bg-white/90 p-5 dark:border-stone-800 dark:bg-black/60">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                <div className="text-sm font-medium">{formatCurrency(appointment.price, hairSpecialist?.currency || 'USD')}</div>
              </div>
