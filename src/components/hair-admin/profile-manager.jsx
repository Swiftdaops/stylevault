                ...
                return (
                  <div key={platform.key}>
                    ...
                    <div className="mt-2 flex gap-2">
                      <input
                        className={`${inputClassName} mt-0 ${platform.key === 'whatsapp' ? 'opacity-70' : ''}`}
                        value={platform.key === 'whatsapp' ? buildWhatsAppUrl(form.whatsapp) : form.socialLinks[platform.key]}
                        onChange={(event) => platform.key === 'whatsapp' ? setField('whatsapp', event.target.value) : setSocialField(platform.key, event.target.value)}
                        placeholder={platform.key === 'whatsapp' ? 'Auto-generated from WhatsApp number' : `${platform.label.toLowerCase()}.com/yourhandle`}
                        readOnly={platform.key === 'whatsapp'}
                      />
                      <a
                        href={href || undefined}
                        target="_blank"
