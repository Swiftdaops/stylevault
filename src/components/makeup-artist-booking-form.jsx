...
import { API_BASE_URL, formatCurrency } from '@/lib/makeup-artist-api';
...
export default function MakeupArtistBookingForm({ makeupArtist, services }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (!makeupArtist?._id) throw new Error('Makeup artist profile is unavailable.');
      ...
      const response = await fetch(`${API_BASE_URL}/makeup-appointments`, {
        method: 'POST',
      ...
      saveStoredCustomerBooking({
        ...
        storeUrl: getMakeupArtistStoreUrl(makeupArtist?.slug),
        providerType: 'makeup-artist',
        providerName: makeupArtist?.name,
        providerSlug: makeupArtist?.slug,
      ...
      setSuccess(`Your request has been sent to ${makeupArtist.name}. We will notify ${form.customerEmail} once the booking is confirmed.`);
      setBookingSummary({
        appName: makeupArtist.name,
        serviceName: selectedService.name,
        appointmentDate: form.date,
        appointmentTime: form.time,
      });
      ...
      toast.success('Booking request sent', {
        description: `${selectedService.name} request sent to ${makeupArtist.name}. Redirecting you to your booking details.`,
      })
      ...
      const whatsappUrl = buildWhatsAppUrl(makeupArtist?.whatsapp, form.customerName)
    ...
    <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
      <form className="rounded-3xl border-[3px] border-rose-200 bg-white p-6 shadow-xl dark:border-stone-800 dark:bg-stone-950" onSubmit={handleSubmit}>
        <div className="mb-6 space-y-2">
          ...
          <h1 className="text-3xl font-bold tracking-tight">Book with {makeupArtist.name}</h1>
          <p className="text-sm text-stone-600 dark:text-rose-200">Choose your makeup service, preferred option, and add-ons to reserve your appointment.</p>
        </div>
            ...
            <select id="service" value={form.service} onChange={handleChange} className="w-full rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 outline-none transition focus:border-rose-400 dark:border-stone-700 dark:bg-stone-900">
              {serviceOptions.map((service) => (
                <option key={service._id} value={service._id}>
                  {service.name} · from {formatCurrency(service.price, makeupArtist?.currency || 'USD')}
                </option>
              ))}
            </select>
          </label>
              ...
              <select id="selectedPricingOption" value={form.selectedPricingOption} onChange={handleChange} className="w-full rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 outline-none transition focus:border-rose-400 dark:border-stone-700 dark:bg-stone-900">
                ...
                {selectedService.pricingOptions.map((option) => (
                  <option key={option.label} value={option.label}>
                    {option.label} · {formatCurrency(option.price, makeupArtist?.currency || 'USD')}
                  </option>
                ))}
              </select>
            </label>
                ...
                {selectedService.addOns.map((addOn) => (
                  <label key={addOn.name} className="flex items-start gap-3 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm dark:border-stone-700 dark:bg-stone-900">
                    ...
                    <span>
                      ...
                      <span className="text-stone-600 dark:text-rose-300">{formatCurrency(addOn.price, makeupArtist?.currency || 'USD')} · +{addOn.duration || 0} min</span>
                    </span>
                  </label>
                ))}
              </div>
        ...
        <div className="mt-5 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-stone-700 dark:border-stone-800 dark:bg-stone-900 dark:text-rose-200">Total: <span className="font-semibold">{formatCurrency(totalPrice, makeupArtist?.currency || 'USD')}</span></div>
        ...
        {whatsAppHref ? (
          <div className="mt-4 rounded-2xl border border-rose-300 bg-rose-50 px-4 py-3 text-sm text-rose-800 dark:border-rose-900 dark:bg-rose-950/40 dark:text-rose-300">
            <p className="font-medium">Need to message {bookingSummary?.appName || makeupArtist.name} directly?</p>
      ...
      <aside className="space-y-6 rounded-3xl border border-rose-200 bg-white/80 p-6 shadow-sm dark:border-stone-800 dark:bg-stone-950/60">
        <div>
          ...
          <h2 className="mt-2 text-3xl font-bold tracking-tight">{makeupArtist.name}</h2>
          <p className="mt-3 text-sm text-stone-600 dark:text-rose-200">{makeupArtist.bio || 'Book directly with this makeup artist.'}</p>
          <div className="mt-4 flex flex-wrap gap-3 text-sm text-stone-600 dark:text-rose-200">
            ...
            <span className="rounded-full border border-rose-200 px-3 py-1 capitalize dark:border-stone-700">{makeupArtist.subscriptionPlan} plan</span>
          </div>
        </div>
        ...
        <div>
          ...
          {selectedService ? (
            <div className="mt-3 rounded-2xl border border-rose-200 bg-rose-50 p-4 dark:border-stone-800 dark:bg-stone-900">
              ...
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-stone-600 dark:text-rose-200">{selectedService.description || 'Professional makeup studio service.'}</p>
                </div>
                <span className="text-base font-semibold">{formatCurrency(totalPrice, makeupArtist.currency || 'USD')}</span>
              </div>
            ...
            <p className="mt-3 text-sm text-stone-600 dark:text-rose-200">No services are configured for this artist yet.</p>
          )}
        </div>

        <div>
          <p className="text-sm uppercase tracking-[0.2em] text-stone-500 dark:text-rose-300">Need more details?</p>
          <Link href={getMakeupArtistStoreUrl(makeupArtist.slug)} className="mt-3 inline-flex rounded-full border border-rose-300 px-4 py-2 text-sm font-medium transition hover:bg-rose-100 dark:border-stone-700 dark:hover:bg-stone-900">View artist page</Link>
        </div>
      </aside>
    </div>
  );
