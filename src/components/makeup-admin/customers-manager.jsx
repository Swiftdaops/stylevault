...
import { getMyMakeupCustomers, updateMyMakeupCustomer } from '@/lib/makeup-artist-api'

export default function MakeupCustomersManager() {
  ...
  const loadCustomers = useCallback(async () => {
    setLoading(true)
    const data = await getMyMakeupCustomers()
  ...
  const save = async () => {
    await updateMyMakeupCustomer(editingId, draft)
    setEditingId(null)
    await loadCustomers()
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-stone-600 dark:text-rose-300">Only your own makeup clients appear here.</p>
      </div>
