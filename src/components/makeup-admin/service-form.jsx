...
import { API_BASE_URL, getMakeupServiceCatalog } from '@/lib/makeup-artist-api'
...
export default function MakeupServiceForm({ makeupArtist, mode = 'create', initialValue = null, onCancel, onSubmit, saving = false, error = '' }) {
  ...
  useEffect(() => {
    async function loadCatalog() {
      ...
      const data = await getMakeupServiceCatalog()
    ...
    <form onSubmit={handleSubmit} className="space-y-5 rounded-3xl border border-rose-200/70 bg-white/90 p-6 shadow-sm dark:border-stone-800 dark:bg-stone-950/70">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <div className="text-sm text-stone-600 dark:text-rose-300">Add makeup services, pricing tiers, and add-ons for your booking page.</div>
        </div>
      ...
      <div>
        ...
        <Select value={form.catalogId} onValueChange={handleCatalogChange}>
          ...
          <SelectContent className="bg-rose-50">
            {catalog.map((item) => (
              <SelectItem key={item._id} value={item._id} className="bg-rose-50">{item.category} — {item.name}</SelectItem>
            ))}
          </SelectContent>
        </Select>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_220px]">
        <div className="space-y-4">
              ...
              <Select value={form.category} onValueChange={(v) => setForm((current) => ({ ...current, category: v }))}>
                ...
                <SelectContent className="bg-rose-50">
                  {categoryOptions.map((option) => (
                    <SelectItem key={option} value={option} className="bg-rose-50">{option}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium">Starting price ({makeupArtist?.currency || 'USD'})</label>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <textarea className={`${textareaClassName} min-h-28 text-sm`} value={form.pricingOptionsText} onChange={(e) => setForm((current) => ({ ...current, pricingOptionsText: e.target.value }))} placeholder="Soft Glam|65.00|90\nBridal Glam|120.00|150" />
            ...
            <div>
              ...
              <textarea className={`${textareaClassName} min-h-28 text-sm`} value={form.addOnsText} onChange={(e) => setForm((current) => ({ ...current, addOnsText: e.target.value }))} placeholder="Strip Lashes|10.00|10\nTouch-Up Kit|15.00|5" />
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-stone-500 dark:text-rose-400">This image is shown on your public makeup booking page. JPG, PNG, WEBP, or SVG up to {MAX_IMAGE_SIZE_LABEL}.</p>
        ...
        <div className="space-y-3 rounded-2xl border border-rose-200/70 bg-rose-50/80 p-4 dark:border-stone-800 dark:bg-stone-950">
          ...
          {selectedCatalogItem?.image && <div className="text-xs text-stone-500 dark:text-rose-400">Template image available; your sample image will override it publicly.</div>}
        </div>
      </div>
