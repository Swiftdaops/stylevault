  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      saveStoredCustomerBooking({
        ...
        storeUrl: getLashTechnicianStoreUrl(lashTechnician?.slug),
        providerType: 'lash-technician',
        providerName: lashTechnician?.name,
        providerSlug: lashTechnician?.slug,
      ...
      toast.success('Booking request sent', {
        description: `${selectedService.name} request sent to ${lashTechnician.name}. Redirecting you to your booking details.`,
      })
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
      <form className="rounded-3xl border-[3px] border-violet-200 bg-white p-6 shadow-xl dark:border-stone-800 dark:bg-stone-950" onSubmit={handleSubmit}>
        ...
        {manageLink ? (
          <div className="mt-4 rounded-2xl border border-violet-300 bg-violet-50 px-4 py-3 text-sm text-violet-900 dark:border-violet-900 dark:bg-violet-950/40 dark:text-violet-200">
