...
import { checkMakeupArtistEmailAvailability, registerMakeupArtist } from '@/lib/makeup-artist-api'
...
export default function MakeupArtistSignupForm({ initialCountry = 'CA' }) {
  ...
  const { emailAvailability, checkEmailAvailability, resetEmailAvailability } = useEmailAvailability({
    checkAvailability: checkMakeupArtistEmailAvailability,
    setFieldError,
  })
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const slug = slugifyBrandName(name || email.split('@')[0] || 'makeup-artist')
    ...
    <div className="mx-auto max-w-md rounded-2xl border border-rose-200/70 bg-white/90 p-6 shadow-sm dark:border-stone-800 dark:bg-black/70">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          }} className="mt-1 w-full rounded-md border px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-rose-300 dark:border-stone-700 dark:bg-stone-900" aria-invalid={Boolean(fieldErrors.email)} />
